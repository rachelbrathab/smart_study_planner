...
class MainModelTrainingStep:
    """Step 17: train and evaluate Random Forest main model."""
    ...
    model_output: Path = MODELS_DIR / "main_random_forest.joblib"
    preds_output: Path = REPORTS_DIR / "step_17_validation_predictions.csv"
    report_output: Path = REPORTS_DIR / "step_17_training.json"
    ...
    def run(self) -> Dict[str, object]:
        ...
        logger = get_logger("ml_pipeline.step_17_training")
        ...
        logger.info("Step 17 completed. Main model saved to %s", self.model_output)
        logger.info("Step 17 completed. Main model report saved to %s", self.report_output)
        return report


if __name__ == "__main__":
    step = MainModelTrainingStep()
    output = step.run()
    print("Step 17 output:")
